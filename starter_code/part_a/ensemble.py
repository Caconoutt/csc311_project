...
def load_data() -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    train = np.array([train['question_id'], train['user_id'], train['is_correct']]).T
    val_data = np.array([val['question_id'], val['user_id']]).T
    val_labels = np.array(val['is_correct'])
    test_data = np.array([test['question_id'], test['user_id']]).T
